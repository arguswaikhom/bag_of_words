...
class HomePageController {
  ...
  static getInstance() {
    if (_instance == null) return HomePageController();
    return _instance;
  }
  ...
  Future<DayStat> fetchDayStat(final String day) async {
    FirebaseUser user = await Application.getInstance().getFirebaseUser();
  ...
  Future<Definition> fetchDefinition(
    ...
    final String query = includeCall
        ? "?ic=True&uid=${(await Application.getInstance().getFirebaseUser()).uid}"
        : "";
  ...
  Future<DefinitionsResponse> fetchLearnedDefinitions({
    ...
    final uid = (await Application.getInstance().getFirebaseUser()).uid;
